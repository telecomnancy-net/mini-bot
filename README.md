...
Il permet au bureau du Mini Tel’ de récolter des citations pour les catégories Citations d’élèves, Citations de profs et Vu/Pas Vu/Entendu/Pas Entendu.
...
1. **Configuration initiale**
   - Avant d'utiliser le bot, assurez-vous d’avoir défini les éléments suivants :
     ...
     - Un fichier `channelID.txt` qui peut être initialement vide ;
     - Définissez les IDs des canaux appropriés dans le code (`channelCitationsID`) ou via la commande `setchannel`.

2. **Exécution du bot**
   - Lancez le fichier `bot.py` pour lancer le bot Discord.
...
- Base du bot et inspiration : Thomas LERUEZ
- Autrice principale : Pauline J.
- Autres collaborateurs : Tom LOISIL

Pour toute question ou assistance, veuillez me contacter à @boufty via Discord.
